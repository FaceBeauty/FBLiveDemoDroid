...
public class PermissionsUtils {
    ...
    public boolean chekPermissions(Activity context, String[] permissions, @NonNull IPermissionsResult permissionsResult) {
        ...
        if(mPermissionList.size() == 0){
            isforbitPermission(false);
        }

        //申请权限
        if ((mPermissionList.size() > 0) && !isforbitPermission()) {//有权限没有通过，需要申请

            ActivityCompat.requestPermissions(context, permissions, mRequestCode);
        } else if(isforbitPermission()) {//用户已经拒绝过一次，不再申请
    ...
    public static boolean isforbitPermission() {
        return SharedPreferencesUtil
            .get("forbitPermission",
                false);
    }

    public static void isforbitPermission(boolean isForbit) {
        SharedPreferencesUtil.put("forbitPermission",
            isForbit);
    }
