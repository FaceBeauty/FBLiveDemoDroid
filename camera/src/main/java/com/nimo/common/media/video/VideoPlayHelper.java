...
import com.nimo.common.utils.MediaFileUtil;
import com.nimo.common.utils.VideoDecoder;
    ...
    public void playVideo(Context context, String path) {
        mPlayerHandler.post(() -> {
            ...
            if (MediaFileUtil.isImageFileType(path)) {
                ...
                int orientation = FileUtils.getPhotoOrientation(path);
...
//                mVideoDecoder.stop();
    ...
    public void playAssetsVideo(Context context, String path) {
        mPlayerHandler.post(() -> {
            ...
            if (MediaFileUtil.isImageFileType(mFilePath)) {
                ...
                int orientation = FileUtils.getPhotoOrientation(mFilePath);
                bitmap = rotateBitmap(bitmap, orientation);
                byte[] rgbBytes = new byte[bitmap.getByteCount()];
                ByteBuffer rgbaBuffer = ByteBuffer.wrap(rgbBytes);
                bitmap.copyPixelsToBuffer(rgbaBuffer);
     ...
     * @param filterFrame
     */
    public void setFilterFrame(int filterFrame) {
        mVideoDecoder.setFilterFrame(filterFrame);
    }
}
