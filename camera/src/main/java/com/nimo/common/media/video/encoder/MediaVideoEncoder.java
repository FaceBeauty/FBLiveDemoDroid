package com.nimo.common.media.video.encoder;
...
public class MediaVideoEncoder extends MediaEncoder {
    ...
    public boolean frameAvailableSoon(int texId, float[] texMatrix, float[] mvpMatrix, boolean isFrontCamera) {
        if (program == null) {
            return false;
        }
        ...
        // 先套上原始 texMatrix
        Matrix.multiplyMM(transformMatrix, 0, texMatrix, 0, GlUtil.IDENTITY_MATRIX, 0);

        // 根据前/后摄像头旋转、镜像
        if (isFrontCamera) {
            // 前置: 旋转90° + 水平镜像
            Matrix.rotateM(transformMatrix, 0, -90, 0f, 0f, 1f);
...
//         如果需要保证宽高比一致，可以再在 mvpMatrix 上做 scale 或 crop
//        float[] mvp = GlUtil.changeMvpMatrixCrop(
//                surfaceViewWidth, surfaceViewHeight,
//                textureWidth, textureHeight
//        );

        Log.d("矩阵", "frameAvailableSoon: "+texMatrix+"  "+mvpMatrix);
...
//        program.drawFrame(texId, texMatrix, mvpMatrix);
        program.drawFrame(texId, transformMatrix, mvpMatrix);
