...
// range 范围跨度越大越好，光源足够时FPS较高，预览更流畅，光源不够时FPS较低，亮度更好。
     ...
     */
    fun setZoom(mCamera: Camera?, targetRatio: Float) {
        ...
        val zoom: Int = indexByBinary(zoomRatios, targetRatio * 100)
        if (zoom == parameters.zoom) return
     ...
     * @return
     */
    fun getMaxZoomScale(mCamera: Camera?): Float {
        if (mCamera != null) {
            val parameters: Camera.Parameters = mCamera.parameters
            if (parameters.isZoomSupported) {
                //getZoomRatios源码The list is sorted from small to large
                val zoomRatios = parameters.zoomRatios
                if (zoomRatios != null && zoomRatios.size == parameters.maxZoom + 1) {
                    val minZoom = zoomRatios[0]
                    val maxZoom = zoomRatios[zoomRatios.size - 1].toFloat()
     ...
     * @return
     */
    private fun indexByBinary(ints: List<Int>, target: Float): Int {
        var low = 0
        var high = ints.size - 1
        if (ints.size == 1) return 0
        if (target <= ints[low]) return low
        if (target >= ints[high]) return high
        var middle = 0
        var left: Float
        var right: Float
        while (low <= high) {
            middle = (low + high) / 2
            right = Math.abs(ints[middle + 1] - target)
            left = Math.abs(ints[middle] - target)
            if (right > left) {
                high = middle - 1
            } else {
                low = middle + 1
        ...
        right = Math.abs(ints[middle + 1] - target)
        left = Math.abs(ints[middle] - target)
        return if (right > left) middle else middle + 1
    }
}