...
public class CameraManager extends BaseCameraManager {
    ...
    public CameraManager(@NonNull Activity activity, @NonNull AutoFitGlSurfaceView autoFitGlSurfaceView,
        ...
        //创建纹理;
        textureId = FBGLUtils.getExternalOESTextureID();
    ...
    public void openCamera(int viewWidth, int viewHeight) {
        try {
            ...
            mAutoFitGlSurfaceView.queueEvent(() -> FBEffect.shareInstance().releaseTextureOESRenderer());
    ...
    public void switchCamera(@Facing int facing) {
        if (facing != mFacing) {
            ...
            mAutoFitGlSurfaceView.queueEvent(() -> {
                ...
                showRenderer = new FBPreviewRenderer(mAutoFitGlSurfaceView.getWidth(), mAutoFitGlSurfaceView.getHeight());
                showRenderer.create(!isBackCamera());

                textureToByteBuffer.destroy();
                textureToByteBuffer = new FBTextureToByteBuffer(mAutoFitGlSurfaceView.getWidth(),mAutoFitGlSurfaceView.getHeight(), true);
                textureToByteBuffer.create(!isBackCamera());
            });
            // 打开相机。
